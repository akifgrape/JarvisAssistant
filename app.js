...
const languages = {
  "en-US": {
    ...
    micPermission:
      "Microphone access required. Please allow and refresh the page.",
    voiceNotAvailable:
      "Voice recognition not available. Please refresh the page.",
  },
  "tr-TR": {
    name: "Türkçe",
    ...
    micPermission:
      "Mikrofon erişimi gerekli. Lütfen izin verin ve sayfayı yenileyin.",
...
function updateLanguage(newLang) {
  ...
  if (
    statusDiv.textContent.includes("Ready") ||
    statusDiv.textContent.includes("Hazır")
  ) {
...
async function loadEnvKeys() {
  try {
    ...
    if (response.ok) {
      ...
      console.info(
        "📝 .env file not found - using existing localStorage keys or manual input"
      );
    }
  } catch (error) {
    console.info(
      "📝 .env file not accessible - using existing localStorage keys"
    );
...
function promptForApiKeys() {
  ...
  models.forEach((model) => {
    if (!localStorage.getItem(`${model}_api_key`)) {
      ...
      const apiKey = prompt(
        `Enter your ${modelNames[model]} API key (or leave empty to skip):`
      );
...
async function initializeAPI() {
  ...
  console.log("🔍 Debug - API Keys loaded:", {
    gemini: API_KEYS.gemini
      ? "SET (" + API_KEYS.gemini.substring(0, 10) + "...)"
      : "NOT SET",
...
function updateModelStatus() {
  ...
  if (hasCurrentKey) {
    hideError();
    console.log(
      `✅ ${currentModel.toUpperCase()} API key configured:`,
      hasCurrentKey.substring(0, 10) + "..."
    );
...
async function initializeVoiceRecognition() {
  ...
  try {
    ...
    showError(
      "Failed to initialize voice recognition. Please refresh and try again."
    );
  }
}
...
function handleVoiceInput(spokenText) {
  ...
  if (API_KEYS[currentModel]) {
    ...
    const fallbackResponse =
      "I heard you say: " +
      spokenText +
      ". However, I need an API key to provide AI responses.";
  ...
  if (saved) {
    try {
      ...
      chatHistory.forEach((msg) => {
        ...
        const formattedMessage =
          msg.sender.toLowerCase() === "jarvis"
            ? markdownToHtml(msg.message)
...
async function getGeminiResponse(text, retryCount = 0) {
  ...
  if (timeSinceLastRequest < MIN_REQUEST_INTERVAL && retryCount === 0) {
    ...
    addMessageToChat(
      "System",
      `⏳ Throttling requests to prevent rate limits. Waiting ${Math.ceil(
        waitTime / 1000
      )} seconds...`
    );
...
User: ${text}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 && retryCount < 3) {
        ...
        console.log(
          `⏳ Rate limited. Retrying in ${delay / 1000}s... (attempt ${
            retryCount + 1
          }/3)`
        );

        updateMicState("processing");
        statusDiv.textContent = `⏳ Rate limited, retrying in ${(
          delay / 1000
        ).toFixed(0)}s...`;

        addMessageToChat(
          "System",
          `⏳ API rate limit reached. Retrying in ${(delay / 1000).toFixed(
            0
          )} seconds...`
        );
  ...
  if (linkMatch) {
    ...
    try {
      ...
      if (newWindow) {
        ...
        addMessageToChat(
          "System",
          "Please allow popups for this site or manually visit: " + url
        );
      }
    } catch (error) {
      ...
      addMessageToChat(
        "System",
        "Could not open link automatically. Please visit: " + url
      );
    }
  } else {
...
function handleAIError(error) {
  ...
  if (error.message.includes("401")) {
    ...
    errorMessage =
      "🚨 API rate limit exceeded. Try switching to a different AI model or wait a few minutes before trying again.";

    const availableModels = Object.keys(API_KEYS).filter(
      (model) => API_KEYS[model] && model !== currentModel
    );
    if (availableModels.length > 0) {
      errorMessage += ` Available alternatives: ${availableModels
        .join(", ")
        .toUpperCase()}`;
    }
...
async function init() {
  try {
    ...
    if (!localStorage.getItem("jarvis_used_before")) {
      setTimeout(() => {
        if (!errorDiv.style.display || errorDiv.style.display === "none") {
          ...
          errorDiv.innerHTML =
            "First time? Click the microphone button and allow microphone access when prompted.";
    ...
    if (errorDiv) {
      errorDiv.style.display = "block";
      errorDiv.innerHTML =
        "Voice recognition failed to load. Please refresh the page or check your internet connection.";
    }
